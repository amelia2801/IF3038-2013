...
<body onload="generateDate()">
	<div id="content">
		...
		<div id="isi">
			<?
				$result = mysqli_query($con,"SELECT * FROM user WHERE username='".$_GET['user']."'");
				$row = mysqli_fetch_array($result);
				
				$result2 = mysqli_query($con,"SELECT nama_tugas FROM `tugas` natural join `user` natural join `mengerjakan` WHERE `user`.username='".$_GET['user']."' AND `tugas`.status_tugas='done'");
				$done = mysqli_fetch_array($result2);
				
				$result3 = mysqli_query($con,"SELECT nama_tugas FROM `tugas` natural join `user` natural join `mengerjakan` WHERE `user`.username='".$_GET['user']."' AND status_tugas='in progress'");
